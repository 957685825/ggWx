			...
			return{
				...
				extraPostData:{
	          	...
	          	workEdit:{ //给后端保存或者编辑完成下一步传递的对象
                        ...
                        operator:"add",
                        edtDbId:'',// 新生成的产品才有的字段
                        tplCode:sessionStorage.getItem('heji_templateCode'), //暂时写死,父组件带入
...
//			this.month = this.$route.query.month;
			...
			this.extraPostData.templateCode = sessionStorage.getItem('heji_templateCode');
			this.extraPostData.editCnfName = sessionStorage.getItem('heji_editCnfName');
			this.extraPostData.defDbId = this.defDbId;
			this.extraPostData.userDbId = localStorage.getItem("userDbId");
			this.ImgHashMap = new HashMap(); //图片
			this.textHashMap = new HashMap();//文字
			this.skuName = sessionStorage.getItem('heji_skuName');
			this.skuCode = sessionStorage.getItem('heji_skuCode');
